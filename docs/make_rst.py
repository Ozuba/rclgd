...
import version
# Mocking missing Godot internal dependencies for standalone use
class Ansi(Enum):
    """
...
def force_stderr_color(cap: bool) -> None:
    pass

def force_stdout_color(cap: bool) -> None:
    pass
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        ...
        if not writing_translation:  # Skip for translations to reduce diff.
            ...
            # In rclgd, the path is relative to the root of the repository
            # Since make_rst.py is in docs/, the root is the parent directory.
            root_directory = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
            source_xml_path = os.path.relpath(class_def.filepath, root_directory).replace("\\", "/")
            source_github_url = f"{version.website}/tree/{git_branch}/{source_xml_path}"
            generator_github_url = f"{version.website}/tree/{git_branch}/docs/make_rst.py"

            f.write(".. DO NOT EDIT THIS FILE!!!\n")
            f.write(f".. Generated automatically from {version.name} sources.\n")
...
def make_type(klass: str, state: State) -> str:
    ...
    def resolve_type(link_type: str) -> str:
        if link_type in state.classes:
            ...
            # If not found in internal rclgd classes, it's likely a standard Godot class.
            # We link it to the official Godot documentation instead of printing an error.
            if link_type == "void":
                return "|void|"
            return f"`{link_type} <https://docs.godotengine.org/en/stable/classes/class_{link_type.lower()}.html>`__"
...
def format_text_block(
    ...
    while True:
        ...
        # Tag is a cross-reference or a formatting directive.
                ...
                if link_target == "":
                    ...
                    if (
                        ...
                        if target_class_name in state.classes:
                            ...
                            # If the class is not in rclgd internal classes, link it to the official Godot documentation.
                            ref_url_name = target_class_name.lower()
                            ref_target_name = target_name.lower().replace("_", "-")
                            
                            # Construct external link to Godot docs.
                            # Format: https://docs.godotengine.org/en/stable/classes/class_node.html#class-node-method-add-child
                            tag_text = f"`{link_target} <https://docs.godotengine.org/en/stable/classes/class_{ref_url_name}.html#class-{ref_url_name}-{tag_state.name}-{ref_target_name}>`__"
                            if tag_state.name == "method":
                                tag_text = tag_text.replace(">`__", "()>`__")
                            escape_pre = True
                            escape_post = True
                            
                            # We must update text and pos here, same as the end of the main while loop.
            ...
            # Invalid syntax.
                ...
                if tag_state.closing:
                    ...
                    tag_text = f"[{tag_text}]"
                # SHORTHAND: [ClassName] where the tag name itself is the class.
                elif (tag_state.name[0].isupper() or tag_state.name.startswith("@")) and tag_state.arguments == "":
        ...
        if not inside_code:
            search_end = next_brac_pos if next_brac_pos != -1 else len(post_text)
            to_escape = post_text[:search_end]
            remainder = post_text[search_end:]
            
            # Escape asterisks.
            to_escape = to_escape.replace("*", "\\*")
            
            # Escape underscores if not followed by an alphanumeric character (to avoid breaking snake_case).
            # This matches Godot's original intent but is much safer than a manual loop.
            to_escape = re.sub(r"_(?![a-zA-Z0-9])", r"\_", to_escape)
            
            post_text = to_escape + remainder
